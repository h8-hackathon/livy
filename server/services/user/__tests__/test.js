...
describe("Succes Case For Users Service", () => {
  it("Successfully Get Users", async () => {
    ...
    expect(response.body[0]).toHaveProperty(
      "image",
      "string image url testing purpose"
    );
  ...
  it("Successfully Get Users use params", async () => {
    const response = await request(app).get(
      "/users?type=superadmin&search=ilias&limit=1"
    );
    ...
    expect(responseByType.body[0]).toHaveProperty(
      "dob",
      "2023-03-07T01:19:32.622Z"
    );
    expect(responseByType.body[0]).toHaveProperty(
      "image",
      "string image url testing purpose"
    ...
    expect(responseByName.body[0]).toHaveProperty(
      "dob",
      "2023-03-07T01:19:32.622Z"
    );
    expect(responseByName.body[0]).toHaveProperty(
  ...
  it("Successfully Update Counselor Submission", async () => {
    const response = await request(app)
      .put("/counselor/" + counselorId + "/submissions")
      .send({
        status: "pending",
      });
    expect(response.status).toBe(200);
...
it("Success post user admin", async () => {
  const response = await request(app).post("/users").send({
    token: "",
    role: "admin",
  });
...
it("Success post user counselor", async () => {
  const response = await request(app).post("/users").send({
    token: "",
    role: "counselor",
  });
